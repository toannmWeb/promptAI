...
2. Nếu task liên quan domain cụ thể → đọc `memory-bank/domains/<X>.md`, `memory-bank/features/<X>.md`.
3. List "persistent facts" tự take note (org rules, tech constraints, naming conventions).

## Menu (khả năng — user gõ code hoặc mô tả)

| Code | Khả năng | Workflow / task |
|---|---|---|
| `BP` | Brainstorm patterns / approaches | `.prompts/workflows/brainstorm.md` (TODO) |
